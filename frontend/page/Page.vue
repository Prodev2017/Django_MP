...
export default {
  ...
  methods:{
    ...
    initPage () {
      ...
      //Если пришли на страницу добавления
        ...
        this.page.status=0
					...
					Page.comments_tree({permlink: this.$route.params.permlink}).then(res => {
	...
	updatePlace(place) {
		this.page.position_text = place.formatted_address
		this.page.position = {
    ...
    setPlace() {
  		if (!this.page.position) {
  			...
  			this.center = {
  				lat: parseFloat(this.page.position.latitude),
  				lng: parseFloat(this.page.position.longitude)
  			}
  		}

        this.markers = [{ label:' here', position: this.center }]
    },
    getCoords(){
		let position_text = this.page.position_text

        let geocoder = new google.maps.Geocoder();
        geocoder.geocode({'address': position_text}, function(a,b){
            return a[0]
        })
    },
    tagChanged(tags){
