...
from apps.pages.models import Page, Tag, MasterTag, Comment, Image
...
def create_page_images(page, article):
    soap = BeautifulSoup(page.body, 'html.parser')
    images_soap = soap.find_all('img')

    images = [i.get('src') for i in images_soap]

    if 'image' in article['meta']:
        images.extend(article['meta']['image'])
    ...
    for url in set(images):
        ...
        image.file.save(filename, File(img_temp))
